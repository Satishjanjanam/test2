...
class AnomalyDetectorController:
    """AnomalyDetectorContoller class"""
    ...
    async def controller_method(req_payload):
        ...
        is_validated, entity_mapping = \
            AnomalyDetectorController.model.validate_metric_dimension(
                metric_name, dimensions)

        Logger.info(f"is_validated= {is_validated}")
        Logger.info(f"Primary Metric Mapping= {entity_mapping}")
        Logger.info("*" * 100)
        ...
        if is_validated is True:
            ...
            timeline = AnomalyDetectorController.model.get_timeline(
                time_period)
            # get the data based on SQL query
            data_timeline = AnomalyDetectorController.model.get_data_from_db(
                entity_mapping=entity_mapping, timeline=timeline, limit=-1)
            output = \
                AnomalyDetectorController.model.predict_anomaly_narratives(
                    data_timeline, entity_mapping)

            Logger.info("*" * 50)

        response = AnomalyDetectorController.model.prepare_response_payload(
            output, entity_mapping, is_validated)
