...
class AnomalyDetector(object):
    ...
    def detect_anomaly(self, metric_col_name, 
        ...
        lower_band = lower_band.rename(columns={metric_col_name: 'lower_bound'})
        sma_band = sma_band.rename(columns={metric_col_name: 'sma_bound'})

        bollinger = time_series_data.join(upper_band).join(lower_band).join(sma_band)
        bollinger = bollinger.dropna()
        
        lower_anomaly = bollinger[bollinger[metric_col_name] < bollinger['lower_bound']]
        upper_anomaly = bollinger[bollinger[metric_col_name] > bollinger['upper_bound']]
        ...
        results[time_series_col_name] = results[time_series_col_name].astype(str)
        results['value'] = results['value'].apply(lambda x: round(x,3))
        results['upper_bound'] = results['upper_bound'].apply(lambda x: round(x,3))
        results['lower_bound'] = results['lower_bound'].apply(lambda x: round(x,3))
        results['sma_bound'] = results['sma_bound'].apply(lambda x: round(x,3))
